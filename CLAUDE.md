...
**Correção da nota técnica acima**: a afirmação de que "Claude nunca aparece em nenhuma contagem agregada de contribuidores" estava incompleta. A API REST (`/repos/.../contributors`) e a página `/graphs/contributors` de fato só usam o campo `Author` — mas o **widget "Contributors" da página principal do repositório** (barra lateral direita) usa uma fonte diferente: ele credita o trailer `Co-Authored-By` como um contribuidor à parte, com link pra uma conta real e especial do GitHub (`github.com/claude`, o mesmo padrão usado pra `github.com/copilot`/`github.com/dependabot`). Isso só foi descoberto porque o usuário mandou um screenshot mostrando "Contributors 2" com "claude" listado, mesmo depois da reescrita de histórico acima já ter zerado o trailer em 100% dos commits alcançáveis por qualquer branch (`main` e `preview`, ambas conferidas). A causa era cache do GitHub — não recomputa esse widget instantaneamente após um force-push, e não existe um jeito de forçar isso manualmente.

### 4. Migração completa pra um repositório GitHub novo (2026-07-17)

Como o cache do widget "Contributors" não deu sinal de atualizar, e o usuário pediu explicitamente pra resolver de forma definitiva, o repositório inteiro foi migrado:

1. **`ACrush14/SistemaCondominio` (original) renomeado pra `ACrush14/SistemaCondominio-antigo`** via API do GitHub (`PATCH /repos/{owner}/{repo}`, usando o token OAuth já armazenado pelo Git Credential Manager do Windows — extraído com `git credential fill`, sem precisar pedir um token novo ao usuário).
2. **Repositório novo `ACrush14/SistemaCondominio` criado vazio** (`POST /user/repos`), reaproveitando a mesma descrição e homepage do original.
3. **`git push` de `main` e `preview`** (as mesmas branches já limpas do passo anterior, sem o trailer) pro repositório novo. Confirmado depois, via API (`/contributors`) e via inspeção da própria página HTML do repo novo, que ele nasceu com **"Contributors 1"**, só o usuário — sem nenhum cache herdado do repositório antigo, já que é uma entidade nova no GitHub.
4. **Projeto da Vercel reconectado** (`Settings → Git → Disconnect` do repo antigo, `Connect Git Repository` no novo) — feito manualmente pelo usuário no painel, porque o Vercel CLI nunca tinha sido autenticado nesse notebook especificamente (a autenticação mencionada em outras partes deste arquivo é da sessão no PC; cada máquina guarda seu próprio token do Vercel CLI, não é algo que viaja pelo repositório/Google Drive). Confirmado "Connected Xm ago" apontando pro repo novo, e o site em produção (`sistemacondominio-nine.vercel.app`) continuou no ar normalmente depois da troca.
5. **`ACrush14/SistemaCondominio-antigo` deixado como privado** (não apagado) — reversível, só sai da listagem/busca pública e do widget de Contributors visível a qualquer um.

**Detalhe técnico sobre a extração do token**: `git credential fill` (com `protocol=https` e `host=github.com` no stdin) devolveu um token OAuth (`gho_...`) já armazenado pelo `credential.helper=manager` do Git para Windows, que era o mesmo usado nos `git push` normais da sessão. Verificado via `GET /user` com o header `Authorization: token ...` que o escopo incluía `repo` (suficiente pra criar/renomear/mudar visibilidade de repositório) antes de usá-lo pra qualquer chamada de API.
